...
public class HashTable {
    ...
    private void extractedPut(String key, String value) {
        int hash = getHash(key);
        final HashEntry hashEntry = new HashEntry(key, value);

        if(hash < 0){
            MyException myException = new MyException("El hash es negativo");
    ...
    public String get(String key) {
        return extractedGet(key);
    }

    private String extractedGet(String key) {
        int hash = getHash(key);
        boolean existe = true;
        ...
        MyException myException = new MyException("La key no existe");
        System.out.println(myException);
        return null;
    }
    ...
    private void extractedDrop(String key) {
        int hash = getHash(key);
        if(entries[hash] != null) {
            HashEntry temp = entries[hash];
            while( !temp.key.equals(key))
            ...
            { entries[hash] = null; //Borrar entrada que no colosiona

            }else if(temp.prev != null){
                if(temp.next != null)
                    temp.next.prev = temp.prev;   //Borrar entrada que este en medio o al final con colisiones
